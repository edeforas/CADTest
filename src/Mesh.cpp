...
void Mesh::merge_vertices(double dDistanceTol)
{
	...
	int iNbVertices = _pKernel->nb_vertices();
	...
	for (int i = 0; i < mOld.nb_triangles(); i++)
	{
		...
		assert(newI1 < nb_vertices());
		assert(newI2 < nb_vertices());
		assert(newI3 < nb_vertices());

		add_triangle(newI1, newI2, newI3);
	}
}
//////////////////////////////////////////////////////////////////////////////////
